  ...
  return (
    <div className="relative flex flex-col items-center justify-center pt-12 pb-24 px-6 min-h-[calc(100vh-80px)] overflow-hidden">
      ...
      >
        ...
        >
          ...
          {gameOverResult?.reason === 'timeout'
            ...
            : gameOverResult?.reason === 'opponent_disconnected'
              ? 'Forfeit'
              : gameOverResult?.reason || 'Finished'}
        </div>
        <h1
          className={cn(
            'font-orbitron text-7xl md:text-9xl font-black tracking-tighter italic scale-110',
            resultColor,
            glowClass,
          )}
        >
        ...
        <p className="font-space-grotesk text-on-surface-variant mt-4 text-lg tracking-widest uppercase">
          {resultSubtext}
        </p>
      </motion.div>
      ...
      <div className="relative z-10 grid grid-cols-1 md:grid-cols-12 gap-6 w-full max-w-5xl">
        ...
        >
          <h3 className="font-orbitron text-xs text-on-surface-variant self-start uppercase tracking-widest">
            Final Tactical Map
          </h3>
          <div className="grid grid-cols-3 gap-3 w-64 h-64">
            {finalBoard.map((cell: string, i: number) => (
              <div
                key={i}
                className="bg-surface-container-highest rounded-xl flex items-center justify-center border border-outline-variant/10"
              >
          ...
          <div className="flex gap-4 items-center">
            <div className="flex items-center gap-2">
              <div
                className={cn(
                  ...
                  isWinner || isDraw ? 'bg-primary animate-pulse' : 'bg-primary/50',
                )}
              ></div>
            ...
            <div className="flex items-center gap-2">
              <div
                className={cn(
                  'w-3 h-3 rounded-full',
                  !isWinner && !isDraw ? 'bg-tertiary animate-pulse' : 'bg-secondary/50',
                )}
              ></div>
          ...
          <div className="glass-card rounded-2xl p-6 border border-outline-variant/15 flex flex-col gap-6 flex-grow bg-surface-container-high/40">
            <div className="flex justify-between items-center border-b border-outline-variant/10 pb-4">
              <div className="flex flex-col">
                ...
                <span className="text-2xl font-orbitron text-secondary uppercase">
                  {rating} {ratingText}
                </span>
              </div>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="text-lg font-orbitron text-on-surface">
                  {String(gameOverResult?.moveCount || 0).padStart(2, '0')}
                </span>
              </div>
              <div className="flex justify-between items-center">
