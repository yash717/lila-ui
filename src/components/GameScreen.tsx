...
export const GameScreen: React.FC<GameScreenProps> = ({ onGameEnd }) => {
    ...
    return (
      <div className="max-w-5xl mx-auto px-4 py-8 md:py-12 flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <p className="text-on-surface-variant text-sm font-orbitron uppercase tracking-wider text-center max-w-md mb-6">
          Your mark is set when they join. They should open <span className="text-secondary">Open rooms</span> in the lobby and tap <span className="text-secondary">Join</span> on your listing — or you can copy the room id for them. 😎
        </p>
        {roomCode ? (
          <div className="w-full max-w-lg glass-panel p-6 rounded-2xl border border-primary/25 text-center space-y-3">
            <span className="text-[10px] font-orbitron text-on-surface-variant uppercase tracking-widest block">Room id (same as in Open rooms)</span>
  ...
  const timeLeft = gameState.mode === 'timed' && gameState.turnDeadline > 0
    ...
    <div className="max-w-5xl mx-auto px-4 py-8 md:py-12 flex flex-col items-center">
      ...
      <section className="flex flex-col md:flex-row items-center justify-between gap-8 mb-12 w-full relative">
        <div className={cn(
          ...
          <div className="relative">
            ...
            <div className="absolute -top-2 -right-2 bg-secondary text-on-secondary text-[10px] font-orbitron font-bold px-2 py-0.5 rounded-full">P1</div>
          </div>
          <div>
        ...
        <div className={cn(
          ...
          <div className="relative">
            <div className="w-16 h-16 rounded-xl border-2 border-tertiary/50 bg-surface-container-highest flex items-center justify-center">
              <span className="font-orbitron text-3xl font-black text-tertiary">
                {playerMark === 'X' ? 'O' : 'X'}
              </span>
            </div>
            <div className="absolute -top-2 -left-2 bg-tertiary text-on-tertiary text-[10px] font-orbitron font-bold px-2 py-0.5 rounded-full">P2</div>
          </div>
      ...
      <section className="flex flex-col items-center w-full max-w-md">
        ...
        <div className="mt-12 flex flex-col items-center">
          <div className={cn(
            ...
            <div className={cn(
              ...
              <span className={cn(
                "font-orbitron font-black text-xl",
        ...
        <div className="mt-8 flex gap-3">
          {['🔥', '👏', '😎', '🤯'].map((emoji, i) => (
            <button key={i} className="w-12 h-12 glass-panel rounded-full flex items-center justify-center text-xl hover:scale-110 active:scale-95 transition-transform hover:bg-primary/10">
              {emoji}
            </button>
          ))}
